...
Если isLoginUnique вернет true, addLogin добавляет новый логин в logins и возвращает строку 'Логин успешно добавлен!' /* Принцип единственной ответственности функции - каждая функция делает что-то одно. Это позволяет переиспользовать код и изменять логику работы функции только в одном месте, не затрагивая работу программы в целом.
...
/* Задача не решена.
1. Функция ничего не добавляет в массив, а просто делает консоль логи.
2. То же самое что и в задаче 05_03.js.
просто return login.length >= 4 && login.length <= 16;
и просто return !allLogins.includes(login); Не усложняй. */
...
//console.log(isLoginUnique(logins, 'Poly'));// проверяет, есть ли в массиве, если нет - true
